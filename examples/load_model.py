...
while True:
  ...
  node = {
    ...
    'Transpose': lambda: [graph.transpose(guid_node[deps[0][0]][deps[0][1]], 
                                 perm=tuple(params[1:1+params[0]]), 
                                 shuffle=params[1+params[0]] if 1+params[0] < len(params) else False)],
    'Dropout':   lambda: [graph.dropout(guid_node[deps[0][0]][deps[0][1]])],
